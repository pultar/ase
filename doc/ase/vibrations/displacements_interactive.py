...
# Optimise geometry
atoms.calc = MOPAC(task='1SCF GRADIENTS', relscf=1e-8)
dyn = LBFGS(atoms)
dyn.run(fmax=1e-3)

# Secondary optimisation at higher precision
atoms.calc = MOPAC(task='1SCF GRADIENTS PRECISE', relscf=1e-12)
dyn = FIRE(atoms)
dyn.run(fmax=1e-4)
...
for disp in displacements:
    disp.calc = MOPAC(task='1SCF GRADIENTS PRECISE', relscf=1e-14)
