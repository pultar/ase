...
Float keywords are keyword have float type. Standard rules are applied same
with integer keyword. You can specify OpenMX float keywords by specifying, ::

.. code-block:: python

  from ase.units import Ha
  calc = OpenMX(scf_energycutoff = 150, convergence = 1e-6 * Ha, ...)

scf_energycutoff is correspond to ``scf.energycutoff``. The other arguement
``convergence`` is a standard parameter format referencing GPAW. It acts same
as scf_criterion. However, units are different. ASE uses standard energy unit
as eV, and OpenMX scf.criterion uses the Hatree unit. Thus, one have to specify
the unit explicitly. This keyword and unit thing is applied to every keyword.
For example, command above will be same as specifying, ::

.. code-block:: python

  from ase.units import Ry
  calc = OpenMX(energy_cutoff = 150 * Ry, scf_criterion = 1e-6, ...)

energy_cutoff is correspond to ``scf_energycutoff``. But it is written in
standard format. You can see Rydberg unit is multiplied. More standard
paramters are specified in calculators/openmx/parameter.py. Bool keywords have
boolean format, True or False. This will be translated On or Off when writing
...
.. code-block:: python
  ...
  calc = OpenMX(definition_of_atomic_species=[['H','H5.0-s1>1p1>1','H_CA13'],
                                              ['C','C5.0-s1>1p1>1','C_CA13']])
