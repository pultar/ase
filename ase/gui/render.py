...
class Render:
    ...
    def __init__(self, gui):
        ...
        self.linewidth_widget = ui.SpinBox(
            0.07, start=0.01, end=9.99, step=0.01
        )
        win.add(
            [
                ui.Label(_('Line width')),
                self.linewidth_widget,
                ui.Label(_('Ångström')),
            ]
        )
        ...
        self.camera_widget = ui.ComboBox(
            labels=self.cameras, values=self.cameras
        )
        ...
        win.add(
            [
                self.run_povray_widget,
                self.keep_files_widget,
    ...
    def ok(self, *args):
        ...
        for frame in frames:
            ...
            plotting_var_settings['radii'] = (
                radii_scale * self.gui.get_covalent_radii()
            )
            renderer = write_pov(
                filename,
                atoms,
