        ...
        for kpt in subpath:
            if isinstance(kpt, str):
                if kpt not in special_points:
                    raise KeyError(
                        'No kpoint "{}" among "{}"'.format(
                            kpt, ''.join(special_points)
                        )
                    )
                labelseq.append(kpt)
                continue
...
class BandPath:
    ...
    def interpolate(
        ...
        return BandPath(
            self.cell, kpts, path=path, special_points=special_points
        )
    ...
    def plot(self, **plotkwargs):
        ...
        return bz.bz_plot(
            self.cell, paths=paths, points=self.cartesian_kpts(), **kw
        )
...
def bandpath(
    path, cell, npoints=None, density=None, special_points=None, eps=2e-4
):
...
def get_special_points(cell, lattice=None, eps=2e-4):
    ...
    if isinstance(cell, str):
        warnings.warn(
            'Please call this function with cell as the first ' 'argument'
        )
...
def monkhorst_pack_interpolate(
    ...
    V = np.pad(
        v, [(pad_width, pad_width)] * 3 + [(0, 0)] * (v.ndim - 3), mode='wrap'
    )
