...
class ConvexHull(object):
    ...
    def plot(self, ocv=False, ref=None, ref_energy=None):
        ...
        # scan right
        right = [pair for pair in conc_Emin_pairs if pair[0] > min_points[-1][0]]
        ...
        if ocv:
            ...
            for x in range(1,len(min_points)):
                pot = ((min_points[x][1] - min_points[x-1][1]) / \
                      ((min_points[x][0] - min_points[x-1][0]) * -1)) \
                      + equib_pot
        ...
        if not ocv:
            ...
            # ax[1].set_xlabel(r'$x$ in $Li_xV_{}Cr_{}O_2F$'.format(1-self.conc_fix_value, self.conc_fix_value))
