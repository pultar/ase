...
class Phonons(Displacement):
    ...
    def save_run_results(self, filename="phonons.results.npz"):
        """Save results from :meth:`~ase.Phonons.run` and
        :meth:`~ase.Phonons.read` calls.

        Save the real-space dynamical matrix, force constant matrix, born
        charge corrections to a file using NumPy in ASE units.
        ...
        """
        np.savez_compressed(filename, indices=self.indices,
                            m_inv_x=self.m_inv_x, cell=self.atoms.cell,
...
class PhononsFromFile:
    """Class for reading saved results generated from
    :class:`ase.phonons.Phonons`.

    Construct a object with restricted method exposure to
    :class:`~ase.phonons.Phonons` from a NumPy zip archive file generated by
    :meth:`~ase.phonons.Phonons.save_run_results`.
    ...
    def __init__(self, filename, atoms=None):
        ...
        if atoms:
            ...
            if not np.array_equal(self.phonons.atoms.cell, cell):
                warnings.warn("Atoms cell does not match cell saved in file!",
                              UserWarning)
