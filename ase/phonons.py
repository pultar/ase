...
class Phonons(Displacement):
    ...
    def save_run_results(self, filename="phonons.results.npz"):
        ...
        Save the real-space dynamical matrix, force constant matrix, born charge 
        corrections to a file using NumPy in ASE units.

        Parameters:

        filename : str, optional
            The NumPy compressed file archive name.

        """
        np.savez_compressed(filename, indices=self.indices, m_inv_x=self.m_inv_x,
                            cell=self.atoms.cell, supercell=self.supercell,
                            D_N=self.D_N, C_N=self.C_N, Z_avv=self.Z_avv,
                            eps_vv=self.eps_vv)


class PhononsFromFile:
    """Class for reading saved results generated from :class:`ase.phonons.Phonons`.

    Construct a object with restricted method exposure to :class:`~ase.phonons.Phonons`
    from a NumPy zip archive file generated by :meth:`~ase.phonons.Phonons.save_run_results`.

    Notes:

        - This is not a child class of :class:`~ase.phonons.Phonons` because inheriting all 
          methods including from base :class:`~ase.phonons.Displacement` is not desired, 
          as they would be invalid calls. Therefore, only suitable methods are exposed. 
          For example, see :meth:`~ase.phonons.Phonons.band_structure`.
    ...
    def __init__(self, filename, atoms=None):
        """Construct from saved :meth:`~ase.phonons.Phonons.save_run_results` call.
        ...
        if atoms:
            ...
            if not np.array_equal(self.phonons.atoms.cell, cell):
                warn("Atoms cell does not match cell saved in file!", UserWarning)
            self.atoms_given = True
        else:
            # Dummy atoms
            self.phonons = Phonons(ase.Atoms())
