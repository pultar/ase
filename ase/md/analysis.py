...
class DiffusionCoefficient:
    ...
    def plot(self, ax=None, show=False):
        ...
        for segment_no in range(self.no_of_segments):
            ...
            for sym_index in range(self.no_of_types_of_atoms):
                for xyz in range(3):
                    if segment_no == 0:
                        label = f'Species: {self.types_of_atoms[sym_index]} ({xyz_labels[xyz]})'
    ...
    def print_data(self):
        ...
        for sym_index in range(self.no_of_types_of_atoms):
            ...
            print('---')
            for segment_no in range(self.no_of_segments):
                print(r'Segment   %3d:         Diffusion Coefficient = %.10f Å^2/fs; Intercept = %.10f Å^2;' % (segment_no,
                      np.mean(self.slopes[sym_index][segment_no]) * fs_conversion, np.mean(self.intercepts[sym_index][segment_no])))
