        ...
        for segment_no in range(self.no_of_segments):
            ...
            if self.is_molecule:
                ...
                for atom_no in self.atom_indices:
                    com_orig += seg[0].positions[atom_no] / len(
                        self.atom_indices
                    )
    ...
    def plot(self, ax=None, show=False):
        ...
        for segment_no in range(self.no_of_segments):
            ...
            for sym_index in range(self.no_of_types_of_atoms):
                ...
                if segment_no == 0:
                    label = (
                        'Segment Mean : %s' % (self.types_of_atoms[sym_index])
                    )
                ax.plot(
                    graph_timesteps[start:end],
                    line,
        ...
        for sym_index in range(self.no_of_types_of_atoms):
            ...
            for segment_no in range(self.no_of_segments):
                print(
                    ...
                    % (
                        segment_no,
                        np.mean(self.slopes[sym_index][segment_no])
                        * fs_conversion,
