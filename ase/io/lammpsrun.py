...
def element_from_mass(mass):
    """
    This algorithm assumes the elements are sorted in order of ascending mass.
    It then returns the atomic symbol of the closest mass to the input mass.

    :param mass: (float) input mass to find corresponding symbol for.
    :returns: chemical symbol
    :rtype: str
    """
    return chemical_symbols[np.searchsorted(atomic_masses, mass)]
