...
def write_gpumd(
    ...
    if triclinic:
        lines.append(
            (' {}' * 12)[1:].format(
                *atoms.pbc.astype(int), *atoms.cell[:].flatten()
            )
        )
    else:
        lines.append(
            (' {}' * 6)[1:].format(
                *atoms.pbc.astype(int), *atoms.cell.lengths()
            )
        )
...
def load_xyz_input_gpumd(fd, species=None, isotope_masses=None):
    ...
    n_columns = (
        5
        + input_parameters['has_velocity'] * 3
    ...
    if isotope_masses is not None:
        mass_symbols = {
            mass: symbol
            for symbol, masses in isotope_masses.items()
            for mass in masses
        }
    symbols = []
