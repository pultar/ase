...
def _get_frame_positions(fd: IO) -> Tuple[int, int, int, List[int]]:
    ...
    if len(items) == 3:  # DLPoly classic
        ...
        nframes = int(items[3])
        pos = [
            ((natoms * (levcfg + 2) + 4) * i + 3) * record_len
    ...
    *Index* can be integer or a slice.
    ...
    for pos_i in positions:
        ...
        yield read_single_image(
            fd, levcfg, imcon, natoms, is_trajectory=True, symbols=symbols
        )


@reader
def read_dlp4(fd: IO, symbols: Optional[List[str]] = None) -> Atoms:
    ...
    return read_single_image(fd, levcfg, imcon, natoms, is_trajectory, symbols)
...
def read_single_image(
    ...
    for i, line in enumerate(itertools.islice(fd, natoms), 1):
        ...
        if levcfg > 0:
            velocities.append(
                [float(x) * DLP_V_ASE for x in next(fd).split()[:3]]
            )
        if levcfg > 1:
            forces.append([float(x) * DLP_F_ASE for x in next(fd).split()[:3]])
...
def write_dlp4(
    ...
    for i, atom in enumerate(atoms):
        ...
        if levcfg > 0:
            to_write = (
                vels[atom.index, :] if vels is not None else (0.0, 0.0, 0.0)
            )
            print(''.join(map(float_format, to_write)), file=fd)

        if levcfg > 1:
            to_write = (
                forces[atom.index, :] if forces is not None else (0.0, 0.0, 0.0)
            )
            print(''.join(map(float_format, to_write)), file=fd)
