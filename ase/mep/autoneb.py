...
def _forbid_optimizer_string(args: List, kwargs: Dict[str, Any]) -> bool:
    ...
    arg_position = 11
    try:
        if len(args) >= arg_position + 1 and isinstance(
            args[arg_position], str
        ):
            args[11] = {'BFGS': BFGS, 'FIRE': FIRE}[args[arg_position]]

        elif isinstance(kwargs.get('optimizer', None), str):
            kwargs['optimizer'] = {'BFGS': BFGS, 'FIRE': FIRE}[
                kwargs['optimizer']
            ]
        else:
            return False
...
class AutoNEB:
    ...
    def __init__(
        ...
        if interpolate_method not in ['idpp', 'linear']:
            ...
            print(
                'Interpolation method not implementet.',
    ...
    def _execute_one_neb(
        self, exitstack, n_cur, to_run, climb=False, many_steps=False
    ):
        ...
        if self.parallel:
            ...
            for i, j in enumerate(to_run[1:-1]):
                ...
                trajhist = closelater(
                    Trajectory(filename_ref, 'w', self.all_images[j])
                )
                qn.attach(seriel_writer(trajhist, i, num).write)

                traj = closelater(
                    Trajectory(
                        '%s%03d.traj' % (self.prefix, j),
                        'w',
                        self.all_images[j],
                    )
                )
    ...
    def run(self):
        ...
        while n_cur < self.n_max:
            ...
            for j in range(n_cur - 1):
                delta_E = (
                    (e[j + 1] - e[j]) * (e[j + 1] + e[j] - 2 * emin) / 2 / enorm
                )
            ...
            if self.world.rank == 0:
                print(
                    f'Adding image between {jmax} and',
        ...
        d1 = np.linalg.norm(
            self.all_images[peak].get_positions()
            - self.all_images[0].get_positions()
        )
        d2 = np.linalg.norm(
            self.all_images[peak].get_positions()
    ...
    def __initialize__(self):
        ...
        for i in range(len(index_exists)):
            if i != index_exists[i]:
                raise Exception(
                    'Files must be ordered sequentially', 'without gaps.'
                )
        if self.world.rank == 0:
