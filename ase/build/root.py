...
def hcp0001_root(
    symbol, root, size, a=None, c=None, vacuum=None, orthogonal=False
):
...
def point_in_cell_2d(point, cell, eps=1e-8):
    ...
    def tri_area(t1, t2, t3):
        ...
        t3x, t3y = t3[0:2]
        return (
            abs(t1x * (t2y - t3y) + t2x * (t3y - t1y) + t3x * (t1y - t2y)) / 2
        )
    ...
    # point in question.
    pA = (
        tri_area(point, c0, c1)
        + tri_area(point, c1, c2)
        + tri_area(point, c2, c3)
        + tri_area(point, c3, c0)
    )
...
def _root_surface_analysis(primitive_slab, root, eps=1e-8):
    ...
    *root* is the desired root to find, and all below.
    ...
    if len(valid_roots) == 0:
        raise ValueError(
            'Invalid root {} for cell {}'.format(root, cell_vectors)
        )
    int_roots = np.array(
        [
            int(this_root)
    ...
    return cell_points, cell_points[np.nonzero(roots == root)[0][0]], set(
        int_roots[1:]
    )
...
def root_surface_analysis(primitive_slab, root, eps=1e-8):
    ...
    *root* is the desired root to find, and all below."""
    return _root_surface_analysis(
        primitive_slab=primitive_slab, root=root, eps=eps
    )[2]
...
def root_surface(primitive_slab, root, eps=1e-8):
    ...
    *root* should be determined using an analysis tool such as the
    ...
    cell = np.array(
        [np.dot(x, root_rotation) * root_scale for x in cell_vectors]
    )
    ...
    cell_points = [
        ...
        if point_in_cell_2d(point + shift, cell, eps=eps)
    ]
    ...
    del atoms[
        [
            atom.index
            for atom in atoms
            if not point_in_cell_2d(atom.position, atoms.cell, eps=eps)
        ]
    ]

    # Rotate cell back to original orientation
    standard_rotation = [
        [cos(-root_angle), -sin(-root_angle), 0],
    ...
    new_positions = np.array(
        [np.dot(x, standard_rotation) for x in atoms.positions]
    )
