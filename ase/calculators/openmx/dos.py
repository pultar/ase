...
class DOS:
    ...
    def read_dos(
        ...
        if spin_polarization:
            self.dos_dict[key + atom_and_orbital + 'up'] = np.ndarray(
                number_of_lines
            )
            self.dos_dict[key + atom_and_orbital + 'down'] = np.ndarray(
                number_of_lines
            )
            ...
            self.dos_dict[
                key + '_cum_' + atom_and_orbital + 'down'
        ...
        if spin_polarization:
            ...
            for i in range(number_of_lines):
                ...
                self.dos_dict[key + atom_and_orbital][
                    i
                ] = read_nth_to_last_value(line, 2)
