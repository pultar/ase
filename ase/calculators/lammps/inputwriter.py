...
def lammps_create_atoms(fileobj, parameters, atoms, prismobj):
    ...
    if parameters['always_triclinic'] or prismobj.is_skewed():
        fileobj.write(
            'region asecell prism 0.0 {} 0.0 {} 0.0 {} '.format(xhi, yhi, zhi)
        )
        fileobj.write(f'{xy} {xz} {yz} side in units box\n')
    else:
        fileobj.write(
    ...
    fileobj.write('create_box {} asecell\n' ''.format(len(species)))
...
def write_lammps_in(
    ...
    for key in ('neighbor', 'newton'):
        if key in parameters:
            fileobj.write(f'{key} {parameters[key]} \n')
    fileobj.write('\n')
    ...
    if 'kim_interactions' in parameters:
        fileobj.write('{}\n'.format(parameters['kim_interactions']))
