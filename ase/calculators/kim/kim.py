...
def KIMCalculator(extended_kim_id, debug=False,
    ...
    if this_is_a_KIM_MO:
        ...
        if kim_mo_simulator == 'kimpy':
            ...
            parameters = {}
            parameters['pair_style'] = 'kim KIMvirial ' + extended_kim_id.strip() + os.linesep
            ...
            raise KIMCalculatorError('ERROR: Unsupported simulator "%s" requested to run KIM API compliant KIM Models.' % kim_mo_simulator)
    ...
    if extended_kim_id != SM_extended_kim_id:
        raise KIMCalculatorError('ERROR: SM extended KIM ID ("%s") does not match expected value ("%s").' % (SM_extended_kim_id, extended_kim_id))
    ...
    if len(model_defn)==0:
        raise KIMCalculatorError('ERROR: model-defn is an empty list in metadata file of Simulator Model "%s".' % extended_kim_id)
    if "" in model_defn:
        raise KIMCalculatorError('ERROR: model-defn contains one or more empty strings in metadata file of Simulator Model "%s".' % extended_kim_id)
    ...
    if simulator_name == "asap":
        ...
        if supported_units != "ase":
            raise KIMCalculatorError('ERROR: KIM Simulator Model units are "%s", but expected to be "ase" for ASAP.' % supported_units)

        # There should be only one model_defn line
        if len(model_defn) != 1:
            raise KIMCalculatorError('ERROR: model-defn contains %d lines, but should only contain one line for an ASAP model.' % len(model_defn))
        ...
        if unknown_potential:
            raise KIMCalculatorError('ERROR: Unknown model "%s" for simulator ASAP.' % model_defn[0])
        else:
    ...
    ############################################################
        ...
        if (lammps_calculator == 'kimlammpsrun'):
            ...
            # add cross-platform line separation to model definition lines
            model_init = [s + os.linesep for s in model_init] # add cross-platform line separation
            ...
            model_init.insert(0, 'atom_modify map array sort 0 0')     # lookup table
            if not any("atom_style" in s.lower() for s in model_init): # atom style (if needed)
                model_init.insert(0, 'atom_style atomic')
            model_init.insert(0, 'units '+supported_units.strip())     # units
            ...
            raise KIMCalculatorError('ERROR: Unknown LAMMPS calculator: "%s"' % lammps_calculator)
    ...
    ############################################################
        ...
        raise KIMCalculatorError('ERROR: Unsupported simulator; simulator_name = "%s".' % simulator_name)
...
#           speclist = list(calc.get_kim_model_supported_species())
            ...
            raise KIMCalculatorError('ERROR: Unsupported simulator "%s" requested to obtain KIM Model species list.' % kim_mo_simulator)
