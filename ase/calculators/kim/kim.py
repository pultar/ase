...
def _get_kim_api_simulator_model_utility():
    ...
    '''
    try:
       libexec_path = subprocess.check_output(["kim-api-v2-build-config", "--libexec-path"]).strip()
    except:
        raise KIMCalculatorError('ERROR: Unable to obtain libexec-path from KIM API utility.')
...
def _is_simulator_model(model):
    ...
    try:
        ...
        raise KIMCalculatorError('ERROR: Unable to determine whether %s is a Simulator Model.' % model)
    if ret == 0:
        is_sm = True # success; this is a Simulator Model
    elif ret == 1:
        raise KIMCalculatorError('ERROR: Model %s does not appear to be installed.' % model)
    else:
        is_sm = False
    return is_sm
