    ...
    * There is a bug in early versions of scipy that will cause eam.py to
    ...
    def _read_potential(self, fd):
        ...
        if self.form == 'eam':  # single element eam file (aka funcfl)
            ...
            for elem in range(self.Nelements):
                ...
                self.embedded_data[elem] = np.float64(data[d : (d + self.nrho)])
                d += self.nrho
        ...
        for i in range(len(atoms)):  # this is the atom to be embedded
            ...
            for j_index in range(self.Nelements):
                ...
                if not use.any():
                    continue
                pair_energy += (
                    np.sum(self.phi[self.index[i], j_index](r[nearest][use]))
                    / 2.0
                )

                if self.form == 'fs':
                    density = np.sum(
                        self.electron_density[j_index, self.index[i]](
                            r[nearest][use]
                        )
                    )
                else:
    ...
    def plot(self, name=''):
        ...
        if self.form == 'adp':
            ...
            self.multielem_subplot(
                r, self.d, r'$r$', r'Dipole Energy', name, plt
            )

            plt.subplot(nrow, 2, 6)
            self.multielem_subplot(
                r, self.q, r'$r$', r'Quadrupole Energy', name, plt
            )
    ...
    def multielem_subplot(
        self, curvex, curvey, xlabel, ylabel, name, plt, half=True
    ):
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        for i in np.arange(self.Nelements):
