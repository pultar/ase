...
class Siesta(FileIOCalculator):
    ...
    def species(self, atoms):
        ...
        for symbol in symbols:
            if symbol not in default_symbols:
                spec = Species(
                    symbol=symbol, basis_set=self['basis_set'], tag=None
                )
    ...
    def set(self, **kwargs):
        ...
        if 'basis_set' in kwargs:
            ...
            if not (
                isinstance(basis_set, PAOBasisBlock) or basis_set in allowed
            ):
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        with open(filename, 'w') as fd:
            ...
            fd.write(
                format_fdf('PAO.EnergyShift', (self['energy_shift'], 'eV'))
            )
            fd.write('\n')
    ...
    def _write_species(self, fd, atoms):
        ...
        for species_number, spec in enumerate(species):
            ...
            if spec['excess_charge'] is not None:
                ...
                cmd = '{} {} {:.7f}'.format(
                    fractional_command, pseudo_head, fraction
                )
                os.system(cmd)
