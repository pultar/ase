...
class CRYSTAL(FileIOCalculator):
    ...
    def _write_crystal_in(self, outfile):
        ...
        if self.kpts is not None:
            ...
            if ispbc[2]:
                ...
                outfile.write(
                    str(self.kpts[0]) + ' ' + str(self.kpts[1]) + ' 1 \n'
                )
            elif ispbc[0]:
    ...
    def read_results(self):
        ...
        with open(
            os.path.join(self.directory, 'OUTPUT'), encoding='latin-1'
        ) as myfile:
...
class PointChargePotential:
    ...
    def read_pc_corrections(self):
        ...
        with open(os.path.join(self.directory, 'FORCES_CHG.DAT')) as infile:
            lines = infile.readlines()

        e = [
            float(x.split()[-1])
            for x in lines
            if 'SELF-INTERACTION ENERGY(AU)' in x
        ][0]
