...
class CP2K(Calculator):
    ...
    def _create_force_env(self):
        ...
        try:
            # read environement id
            self._force_env_id = int(self._shell.recv())
        except ValueError:
            warn('Invalid environment. Using 1.', RuntimeWarning)
            self._force_env_id = 1
    ...
    def _generate_input(self):
        ...
        for elem in set(self.atoms.get_chemical_symbols()):
            ...
            if potential:
                elem_potential = potential
                # search for a more precise potential name in CP2K < 3
                # read POTENTIAL file path when CP2K_DATA_DIR is set
                if 'CP2K_DATA_DIR' in os.environ and self._shell.version < 3:
                    # potential=GTH-PBE should be updated to e.g. GTH-PBE-q6
                    # read file p.potential_file, look for the element and potential
                    # root name in lines, then get its full name as 2nd word
                    with open(os.path.join(os.environ['CP2K_DATA_DIR'], p.potential_file)) as f:
                        for line in f:
                            line = line.strip() # trim line with spaces
                            # detect line e.g. 'O GHT-PBE*'
                            if line.startswith(elem+' '+potential):
                                elem_potential = line.split()[1]  # 2nd word
                                break # we found it, stop searching
                kinds[elem].keywords.append('POTENTIAL ' + elem_potential)
...
class Cp2kShell(object):
    ...
    def __init__(self, command, debug):
        ...
        if not line.startswith('CP2K Shell Version:'):
            # tolerant with older CP2K versions
            warn('Cannot determine version of CP2K shell.  '
                               'Probably the shell version is too old.  '
                               'Please update to CP2K 3.0 or newer.', RuntimeError)
        else:
