...
def write_read_atoms(atom, tmp_path):
    write(f'{tmp_path}/{"castep_test.cell"}', atom)
    return read(f'{tmp_path}/{"castep_test.cell"}')
...
def test_custom_mass_write(
    ...
    with open(f'{tmp_path}/{"castep_test.cell"}') as f:
        data = f.read().replace("\n", "\\n")
...
def test_custom_mass_overwrite(tmp_path):
    ...
    with pytest.raises(ValueError,
        ...
        atoms.write(f'{tmp_path}/{"castep_test2.cell"}')
