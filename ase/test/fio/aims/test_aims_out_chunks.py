...
eps_hp = (
    1e-15  # The espsilon value used to compare numbers that are high-precision
)
eps_lp = (
    1e-7  # The espsilon value used to compare numbers that are low-precision
)
...
def test_header_transfer_initial_atoms(
    ...
    assert np.allclose(
        empty_calc_chunk.initial_atoms.positions, initial_positions
    )
...
def test_calc_forces(calc_chunk):
    ...
    assert np.allclose(
        calc_chunk.atoms.get_forces(apply_constraint=False), forces
    )
...
def test_calc_free_energy(calc_chunk):
    ...
    assert (
        np.abs(calc_chunk.atoms.calc.get_property('free_energy') - free_energy)
        < eps_hp
    )
...
def test_calc_hirshfeld_charges(calc_chunk):
    ...
    assert np.allclose(
        calc_chunk.results['hirshfeld_charges'], hirshfeld_charges
    )
...
def test_calc_hirshfeld_volumes(calc_chunk):
    ...
    assert np.allclose(
        calc_chunk.results['hirshfeld_volumes'], hirshfeld_volumes
    )
...
def test_calc_hirshfeld_atomic_dipoles(calc_chunk):
    ...
    assert np.allclose(
        calc_chunk.hirshfeld_atomic_dipoles, hirshfeld_atomic_dipoles
    )
    assert np.allclose(
...
def test_calc_eigenvalues(calc_chunk, eigenvalues_occupancies):
    assert np.allclose(
        calc_chunk.eigenvalues, eigenvalues_occupancies[:, :, :, 1]
    )
    assert np.allclose(
...
def test_calc_occupancies(calc_chunk, eigenvalues_occupancies):
    assert np.allclose(
        calc_chunk.occupancies, eigenvalues_occupancies[:, :, :, 0]
    )
    assert np.allclose(
...
def test_molecular_header_initial_atoms(
    ...
    assert np.all(
        ['O', 'H', 'H'] == molecular_header_chunk.initial_atoms.symbols
    )
    assert np.allclose(
...
def test_molecular_calc_atoms(molecular_calc_chunk, molecular_positions):
    ...
    assert np.allclose(
        molecular_calc_chunk.atoms.positions, molecular_positions
    )
...
def test_molecular_calc_forces(molecular_calc_chunk):
    forces = np.array(
        [
            [
                0.502371357164392e-03,
                0.518627676606471e-03,
                0.000000000000000e00,
            ],
            [
                -0.108826758257187e-03,
                -0.408128912334209e-03,
                -0.649037698626122e-27,
            ],
            [
                -0.393544598907207e-03,
                -0.110498764272267e-03,
                -0.973556547939183e-27,
            ],
        ]
    )
...
def test_molecular_calc_free_energy(molecular_calc_chunk):
    ...
    assert (
        np.abs(molecular_calc_chunk.results['free_energy'] - free_energy)
        < eps_hp
    )
    assert (
        np.abs(
            molecular_calc_chunk.atoms.calc.get_property('free_energy')
            - free_energy
        )
        < eps_hp
    )
...
def test_molecular_calc_energy(molecular_calc_chunk):
    ...
    assert (
        np.abs(molecular_calc_chunk.atoms.get_potential_energy() - energy)
        < eps_hp
    )
...
def test_molecular_calc_hirshfeld_charges(
    ...
    assert np.allclose(
        molecular_calc_chunk.results['hirshfeld_charges'],
...
def test_molecular_calc_hirshfeld_volumes(molecular_calc_chunk):
    ...
    assert np.allclose(
        molecular_calc_chunk.hirshfeld_volumes, hirshfeld_volumes
    )
    assert np.allclose(
...
def test_molecular_calc_hirshfeld_dipole(
    ...
    hirshfeld_dipole = np.sum(
        molecular_hirshfeld_charges.reshape((-1, 1)) * molecular_positions,
        axis=1,
    )
