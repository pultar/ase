...
def test_write_bs_xml(tmp_path, nitrogen_trioxide_atoms, excitingtools):
...
def test_write_dos_xml(tmp_path, nitrogen_trioxide_atoms, excitingtools):
...
def test_parse_info_out_xml_bad_path(tmp_path, excitingtools):
    ...
    with pytest.raises(FileNotFoundError):
        ase.io.exciting.parse_output(output_file_path)
...
def test_parse_info_out_energy(tmp_path, excitingtools):
    ...
    assert (
        pytest.approx(
            float(
                results['scl'][final_scl_iteration]['Estimated fundamental gap']
            )
        )
        == 0.36095838
    )
    ...
    assert (
        pytest.approx(float(initialization['Unit cell volume']))
        == 4412.7512103067
    )
    ...
    # to lattice vectors.
    lattice_vectors_as_matrix = np.array(
        np.reshape(initialization['Lattice vectors (cartesian)'], (3, 3), 'F'),
        float,
    )
