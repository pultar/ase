...
def lammpsdump():
    def factory(bounds="pp pp pp",
        ...
        buf = """\
            ...
            3
            ITEM: BOX BOUNDS _bounds_
            0.0e+00 4e+00
            0.0e+00 5.0e+00
            0.0e+00 2.0e+01
            ITEM: ATOMS _element_ _id_ _type_ _pos_cols_
            C  1 1 0.5 0.6 0.7
            C  3 1 0.6 0.1 1.9
        ...
        for field, val in [
            ("_bounds_", bounds),
            ("_pos_cols_", position_cols),
            ("_element_", "element" if have_element else "unk0"),
            ("_id_", "id" if have_id else "unk1"),
            ("_type_", "type" if have_type else "unk2"),
        ]:
            buf = buf.replace(field, val)
