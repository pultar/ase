...
def _base_calculator_gpwfile(tmp_path_factory, factories):
    ...
    def __base_calculator_gpwfile(
        atoms, filename, nbands, gpts=(8, 8, 8), kpts=(Nk, Nk, Nk)
    ):
...
def _h2_calculator_gpwfile(_base_calculator_gpwfile):
    ...
    gpw_path = _base_calculator_gpwfile(
        atoms=atoms, filename='wan_h2.gpw', nbands=4
    )
    return gpw_path
...
def _si_calculator_gpwfile(_base_calculator_gpwfile):
    atoms = bulk('Si')
    gpw_path = _base_calculator_gpwfile(
        atoms=atoms, filename='wan_si.gpw', nbands=8
    )
    return gpw_path
...
def _ti_calculator_gpwfile(_base_calculator_gpwfile):
    ...
    gpw_path = _base_calculator_gpwfile(
        atoms=atoms, filename='wan_ti.gpw', nbands=None
    )
    return gpw_path
...
def test_get_radii(lat, wan):
    ...
    if (lat.tocell() == FCC(a=1).tocell()).all() or (
        ...
        pytest.skip(
            'Lattices not supported by this function,'
...
def test_write_cube_repeat(wan, testdir):
    ...
    with open(cubefilename) as inputfile:
        content = read_cube(inputfile)
    assert (
        pytest.approx(content['atoms'].cell.array)
        == (atoms * repetition).cell.array
    )
    assert pytest.approx(content['data']) == abs(
        wanf.get_function(index, repetition)
    )
...
def test_get_pdos(wan):
    ...
    atoms.get_potential_energy()
    wanf = wan(
        atoms=atoms, calc=calc, nwannier=nwannier, initialwannier='bloch'
    )
...
def test_translate(wan, h2_calculator):
    ...
    wanf = wan(
        nwannier=nwannier, initialwannier='bloch', calc=calc, full_calc=True
    )
...
def test_distances(wan, h2_calculator):
    ...
    for i in range(nwannier):
        ...
        for j in range(i + 1, nwannier):
            ...
            assert dist_ww[i, j] == pytest.approx(
                np.linalg.norm(cent_w[i] - cent_w[j])
            )
...
def test_get_hamiltonian_bloch(wan):
    ...
    wanf = wan(
        atoms=atoms, kpts=kpts, nwannier=nwannier, initialwannier='bloch'
    )
...
def test_get_function(wan):
    ...
    for i in range(nwannier):
        ...
        assert (
            gpts_np * [1, 2, 3]
            == wanf.get_function(index=i, repeat=[1, 2, 3]).shape
        ).all()
...
def test_scdm(ti_calculator):
    ...
    for k in range(number_kpts):
        ...
        assert (
            orthogonality_error(C_kul[k].T) < 1e-10
