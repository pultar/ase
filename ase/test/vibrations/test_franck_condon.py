...
def test_franck_condon(testdir):
    ...
    for m in [2, 7]:
        ...
        equal(
            fcr.ov0mm1(m, delta),
            fcr.ov0m(m, delta) * fcr.ov1m(m, delta),
            1.0e-15,
        )
        ...
        equal(
            fcr.ov0mm2(m, delta),
            fcr.ov0m(m, delta) * fcr.ov2m(m, delta),
            1.0e-15,
        )
        equal(fco.direct0mm2(m, S), fcr.direct0mm2(m, delta), 1.0e-15)

        equal(
            fcr.direct0mm3(m, delta),
            fcr.ov0m(m, delta) * fcr.ov3m(m, delta),
            1.0e-15,
        )

        equal(
            fcr.ov1mm2(m, delta),
            fcr.ov1m(m, delta) * fcr.ov2m(m, delta),
            1.0e-15,
        )
