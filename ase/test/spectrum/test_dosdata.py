...
class TestRawDosData:
    ...
    def test_sample_grid(self, sparse_dos):
        ...
        assert np.allclose(
            default_dos.get_energies(), np.linspace(0.9, 5.3, 10)
        )
    ...
    def test_plot(self, sparse_dos, figure, linewidth):
        ...
        ax_out = sparse_dos.plot(
            npts=5, ax=ax, mplargs=mplargs, smearing='Gauss'
        )
        assert ax_out == ax
...
class TestGridDosData:
    ...
    def dense_dos(self):
        ...
        y = np.sin(x / 10)
        return GridDOSData(
            x, y, info={'symbol': 'C', 'orbital': '2s', 'day': 'Tue'}
        )
    ...
    def another_dense_dos(self):
        ...
        y = np.sin(x / 10) * 2
        return GridDOSData(
            x, y, info={'symbol': 'C', 'orbital': '2p', 'month': 'Feb'}
        )
    ...
    def test_resampling_consistency(self, dense_dos, denser_dos):
        ...
        assert np.allclose(
            from_dense.get_energies(), from_denser.get_energies()
        )
        assert np.allclose(
            from_dense.get_weights(),
    ...
    def test_plot_broad_dos(self, dense_dos, figure):
        ...
        ax = figure.add_subplot(111)
        _ = dense_dos.plot(
            ax=ax, npts=10, xmin=0, xmax=9, width=4, smearing='Gauss'
        )
        line_data = ax.lines[0].get_data()
