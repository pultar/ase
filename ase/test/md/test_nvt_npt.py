...
def equilibrated(asap3, berendsenparams):
    ...
    MaxwellBoltzmannDistribution(
        atoms, temperature_K=100, force_temp=True, rng=rng
    )
    Stationary(atoms)
...
def test_nvtberendsen(asap3, equilibrated, berendsenparams, allraise):
    t, _ = propagate(
        Atoms(equilibrated), asap3, NVTBerendsen, berendsenparams['nvt']
    )
...
def test_nptberendsen(asap3, equilibrated, berendsenparams, allraise):
    t, p = propagate(
        Atoms(equilibrated), asap3, NPTBerendsen, berendsenparams['npt']
    )
