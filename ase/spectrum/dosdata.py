...
class DOSData(metaclass=ABCMeta):
    ...
    def sample_grid(
        ...
        weights_grid = self._sample(
            energies_grid, width=width, smearing=smearing
        )
    ...
    def label_from_info(info: Dict[str, str]):
        ...
        if 'label' in info:
            ...
            return '; '.join(map(lambda x: f'{x[0]}: {x[1]}', info.items()))
...
class GeneralDOSData(DOSData):
    ...
    def __init__(
        self, energies: Floats, weights: Floats, info: Info = None
    ) -> None:
    ...
    *intersection* of the inputs: only key-value pairs that were common to both
    ...
    def __add__(self, other: 'RawDOSData') -> 'RawDOSData':
        if not isinstance(other, RawDOSData):
            raise TypeError(
                'RawDOSData can only be combined with other '
                'RawDOSData objects'
            )
...
class GridDOSData(GeneralDOSData):
    ...
    def __init__(
        ...
        n_entries = len(energies)
        if not np.allclose(
            energies, np.linspace(energies[0], energies[-1], n_entries)
        ):
    ...
    def __add__(self, other: 'GridDOSData') -> 'GridDOSData':
        ...
        if not isinstance(other, GridDOSData):
            raise TypeError(
                'GridDOSData can only be combined with other '
                'GridDOSData objects'
            )
        if len(self._data[0, :]) != len(other.get_energies()):
            raise ValueError(
                'Cannot add GridDOSData objects with different-'
                'length energy grids.'
            )
