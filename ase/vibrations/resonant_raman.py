...
class ResonantRaman(Raman):
    ...
    def read_excitations(self):
        ...
        self.ex0m_pc = (
            np.array([ex.get_dipole_me(form=self.dipole_form) for ex in ex0])
            * u.Bohr
        )
        exmE_rp = []
        expE_rp = []
    ...
    def read_excitations_overlap(self):
        ...
        def load(disp, rep0_p):
            ...
            ov_nn = np.where(
                np.abs(ov_nn) > self.minoverlap['orbitals'], ov_nn, 0
            )
            ov_pp = ex_p.overlap(ov_nn, ex0)
            ov_pp = np.where(
                np.abs(ov_pp) > self.minoverlap['excitations'], ov_pp, 0
            )
