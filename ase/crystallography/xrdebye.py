...
import ase
from ase.crystallography.xrayfunctions import (
...
def one_species_contribution(
    positions: np.ndarray,
    form_factor_spline: InterpolatedUnivariateSpline,
    s_vect: np.ndarray,
) -> np.ndarray:
    r"""Calculates the contributions to the scattered intensity where both 
    form factors belong to the same atomic species.

    This function calculates the distances between all points on the same 
    array using scipy.spatial.distance.pdist. This does not calculate the 
    diagonal elements of the distance matrix, which are all zero. The term 
    corresponding to the zero distances is explicitly added as n_atoms 
    \times f_i ^2 where f_i is the atomic form factor.

    The evaluated expression is:

    N f_i^2 + 2 \sum_{i,j>i}^N f_i^2 sinc(2 s r_{ij})

    Only the strictly upper right half of the distance matrix for the given 
    positions is ever calculated. The s r_{ij} = 0 terms are in the N f_i^2 
    term (sinc(0) = 1) and the sum over nontrivial scatterings is multiplied 
    by two to account for the strictly lower left half of the distance matrix.
...
def one_species_hist_contribution(
    ...
    for distances in pdist_in_chunks(positions):
        ...
        for i in range(s_vect.shape[0]):
            contribution[i] += form_factor_spline(s_vect[i]) ** 2 * (
                n_atoms
                + 2 * np.sum(dist_hist * np.sinc(bin_edges * s_vect[i] * 2))
            )
...
def two_species_contribution(
    ...
    r"""Calculates the contributions to the scattered intensity where the 
    form factors belong to different atomic species.

    This function calculates the distances between all points on two different 
    arrays using scipy.spatial.distance.cdist. The corresponding (in general 
    rectangular) distance matrix will have no nonzero entries unless two atoms 
    of different species have the exact same position (unphysical inputs).

    The evaluated expression is:

    2 \sum_i^N \sum_j^N f_i f_j sinc(2 s r_{ij})

    The entire distance matrix is actually an off-diagonal block of the full 
    distance matrix for the entire system. There is a corresponding 
    off-diagonal block in the other half of the symmetric distance matrix. 
    The multiplication by 2 accounts for the second identical block.
...
class XrayDebye:
    ...
    def __init__(
        ...
        if histogram_approximation:
            self.one_species_contribution = partial(
                one_species_hist_contribution, bin_width=bin_width
            )
            self.two_species_contribution = partial(
                two_species_hist_contribution, bin_width=bin_width
            )
        else:
    ...
    def calc_pattern(self, x=None, mode="XRD"):
        ...
        Returns:
            Array of intensities calculated for values given in ``x``.
        """
